...
TrueWatch RUM 用于采集商城页面的访问性能、Resource、Error、Browser Logs 与 Session Replay，并与后端 Trace 关联。

### 步骤一：创建新的客户端应用

按照以下步骤在 TrueWatch 创建 Web 类型的客户端应用：
...
下一章会通过 Helm 将 RUM Application ID 和 TrueWatch Workspace ID 传入应用。

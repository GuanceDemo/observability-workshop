...
進入 **日誌 → Pipelines**，點選 **新建 Pipeline**。
...
選擇 **手動編寫** 模式，將以下 Pipeline 指令碼貼到 **定義解析規則**，然後點選 **開始測試**：
...
在 **返回結果** 中確認日誌已提取出 `trace_id`、`service`、`key_request`、`biz_request_id`、`language`、`fault_id`、`pod_name` 等欄位。業務正文儲存在 `log_message`，完整原始內容繼續儲存在 `message`。

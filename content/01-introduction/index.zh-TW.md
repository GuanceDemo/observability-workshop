...
本工作坊將透過在 AWS EKS（Elastic Kubernetes Service）中部署並監控一個可注入故障的微服務商城 Demo，介紹觀測雲可觀測性平台的實際應用。目標是為學員提供完整的實操體驗，涵蓋 Kubernetes 與容器指標、應用效能監控、日誌分析、JVM 監控、Profiling、使用者體驗監控、告警管理及端到端鏈路追蹤等核心功能。
...
本工作坊將帶領您在 AWS EKS 叢集中安裝 DataKit、部署微服務商城 Demo、產生業務流量、注入故障，並透過觀測雲完成從使用者存取到後端服務及基礎設施的全鏈路觀測與問題分析。

### 觀測雲可觀測性平台功能特色

觀測雲是全面的可觀測性解決方案，提供應用效能、基礎設施健康度、使用者行為監控（RUM）、端到端追蹤、日誌分析及告警管理等多樣化功能。主要特色包括：
...
1. **觀測雲 Cloud 帳號：**
   - 註冊 [https://auth.guance.com/businessRegister](https://auth.guance.com/businessRegister) 並建立工作區 (Workspace)，預設為商業版。
2. **AWS EKS 叢集：**
...
**注意事項：**  
觀測雲透過「工作區」將不同環境或應用的觀測資料分離，多個環境間可透過授權方式共享資料。DataWay URL 中包含敏感 token，請勿將其寫入文件、截圖或 Git 儲存庫。
